...
import { getAllLedger, getCustomerLedger, addCredit, deleteCustomer } from '../api/ledgerApi';
        ...
        <div style={{ background: '#fff', color: '#111', fontFamily: 'Georgia, serif', padding: '2rem', minWidth: 500 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', fontSize: '0.8rem', borderTop: '1px solid #ddd', paddingTop: '1rem', marginBottom: '1.5rem' }}>
                <div>
                    ...
                    <div><b>Bill to:</b> {invoice.client_name}</div>
...
export default function Ledger() {
    ...
    const handleCustomerClick = async (customer) => {
        ...
        const res = await getCustomerLedger(customer.customer_name);
        setEntries(res.data);
        setShowDetail(true);
        setShowCredit(false);
    };
    ...
    const handleDeleteCustomer = async (customerName) => {
        if (!window.confirm(`Delete all data for '${customerName}'? This includes all invoices and ledger entries.`)) return;
        try {
        ...
        <div className="max-w-5xl mx-auto p-6">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm">{ledger.length} customers</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-12 text-gray-400 bg-gray-50 rounded-lg">
                    No ledger entries yet. Create an invoice first!
                </div>
            ) : (
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                {['Customer', 'Invoices', 'Last Transaction', 'Total Debit', 'Total Credit', 'Debt', 'Actions'].map(h => (
                        ...
                        <tbody>
                            {ledger.map((row, i) => (
                                <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                                    <td className="px-4 py-3 font-semibold text-gray-900">{row.customer_name}</td>
            ...
            {showDetail && selected && (
                <div className="border border-gray-200 rounded-lg p-5 bg-white">
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <div>
                            <h3 className="text-lg font-extrabold">{selected.customer_name}</h3>
