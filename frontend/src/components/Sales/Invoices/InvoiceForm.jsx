...
//   const [client, setClient] = useState({ client_name: '', phone: '', address: '' });
...
//     if (!client.client_name) return alert('Please enter client name!');
//     if (items.some(i => !i.description || !i.price)) return alert('Fill all item fields!');

//     // ⚠️ Check stock before saving
...
//     setLoading(true);
//     try {
//       await createInvoice({ ...client, items });
...
//         <h3 className="font-bold mb-3 text-sm text-gray-600 uppercase tracking-wider">Client Info</h3>
//         <input className={`${inp} mb-2`} placeholder="Client Name *"
//           value={client.client_name} onChange={e => setClient({ ...client, client_name: e.target.value })} />
